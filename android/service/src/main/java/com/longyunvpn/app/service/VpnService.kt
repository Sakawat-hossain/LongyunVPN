...
import com.longyunvpn.app.common.R as CommonR
import com.longyunvpn.app.core.Core
import com.longyunvpn.app.service.models.CIDR
...
import com.longyunvpn.app.service.modules.ServiceModules
...
class VpnService : SystemVpnService(), ManagedService {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        // Android starts always-on VPN through this callback instead of LongyunVPN's bound-service
    ...
    // Entries are added one at a time. Both calls throw NameNotFoundException
    // for a package that is no longer installed, and start() turns any exception
    // into a failed start - so a single stale entry in the user's app list used
    // to take the whole tunnel down, with nothing on screen to say why. Lists
    // outlive installs, and the launcher-only <queries> narrows what can be
    // resolved, so stale entries are expected rather than exceptional.
        ...
        when (accessControl.mode) {
            AccessControlMode.ACCEPT_SELECTED -> {
                (accessControl.acceptList + packageName).forEach { name ->
                    addSkippingUninstalled(name) { addAllowedApplication(it) }
                }
            }

            AccessControlMode.REJECT_SELECTED -> {
                (accessControl.rejectList - packageName).forEach { name ->
                    addSkippingUninstalled(name) { addDisallowedApplication(it) }
                }
            }
        }
    }

    private inline fun addSkippingUninstalled(name: String, add: (String) -> Unit) {
        try {
            add(name)
        } catch (_: android.content.pm.PackageManager.NameNotFoundException) {
            GlobalState.log("Access control: skipping uninstalled $name")
        }
    }
