...
import com.longyunvpn.app.service.ManagedService
import com.longyunvpn.app.service.ProxyService
import com.longyunvpn.app.service.ServiceConfig
import com.longyunvpn.app.service.VpnService
...
object ServiceController {
    ...
    @Volatile
    private var serviceLostListener: ((String) -> Unit)? = null

    /**
     * Registers the callback told when the running service is lost without a
     * stop having been asked for. ServicePlugin forwards it to Dart, whose
     * reconnect logic listens for exactly this. Pass null to clear it.
     */
    fun setServiceLostListener(listener: ((String) -> Unit)?) {
        serviceLostListener = listener
    }
    ...
    private fun handleServiceDisconnected(
        ...
        GlobalState.launch {
            ...
            if (wasCurrent) {
                ...
                // Only a live binding counts. stop() clears the binding before
                // the disconnect it causes arrives, so a user-requested stop is
                // recognised as stale above and never reported as a loss.
                serviceLostListener?.invoke(message)
            }
        }
    }
}
